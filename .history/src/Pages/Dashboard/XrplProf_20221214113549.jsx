...
const Xrpl = () => {
  ...
  const [mintingProgress, setMintingProgress] = useState(minting1);
  ...
  useEffect(() => {
    if (walletAddress != "") {
      axios.get(accountStatusUrl + walletAddress).then((response) => {
        if (response.data.message == "User didn't mint yet") return;
        if (response.data.message == "User minted but didn't claim yet") setMintingProgress(minting2);
        if (response.data.message == "User minted and claimed") setMintingProgress(minting3)
      })
    }
  },[walletAddress])
    ...
    <AnimatedPage>
      ...
      <div className="mint-page">
        ...
        <div className="mint-div">
          ...
          <div onClick={mintXrp} to={"/dashboard"} className="mint-btn"><img src={board} alt="" /><p className="btn-text cp">Mint Professor</p></div>
